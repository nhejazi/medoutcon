...
cv_eif <- function(fold,
  ...
  if (!all(data_in$C == 1) || !(all(data_in$two_phase_weights == 1))) {
...
#'
#' @param C A \code{logical} vector indicating whether an unit's mediators were
#'   measured using a two-phase sampling design.
...
#'   measured. These weights should only be provided if \code{C} is specified.
...
two_phase_eif <- function(C,
  ...
  ipw_two_phase <- C * two_phase_weights

  # compute the updated eif
  # NOTE: D_{two-phase} = C/g_C * D_{full-data} -
  #                       (C/g_C - 1) * E[D_{full-data} | ...]
