...
#'  are in the order specified by the NPSEM (Y, M, Z, C, A, W), with column
...
fit_treat_mech <- function(train_data,
  ...
  if (type == "g") {
    ...
    train_data[, obs_weights := C * two_phase_weights * obs_weights]
    valid_data[, obs_weights := C * two_phase_weights * obs_weights]
  }
...
#'  columns in the order specified by the NPSEM (Y, M, Z, C, A, W), with column
...
#'  in the order specified by the NPSEM (Y, M, Z, C, A, W), with column names
...
fit_moc_mech <- function(train_data,
                         ...
                         type = c("q", "r")) {
  # update observation weights with two-phase sampling weights, if necessary
  # TODO: is this necessary? the intermediate confounder is not affected by the
  #       mediators and should not be subject to two-phase sampling. would the
  #       nuisance parameters based on this still need to be re-weighted?
  train_data[, obs_weights := C * two_phase_weights * obs_weights]
  valid_data[, obs_weights := C * two_phase_weights * obs_weights]
...
#' Fit estimated efficient influence function, conditioning on W, A, C, M, Z, Y
#'
...
#' and conditioning on W, A, C, Z and Y. It is used to adjust the full-data
#' efficient influence function for two-phase sampling designs.
#'
#' @param train_data A \code{data.table} containing observed data, with columns
#'   in the order specified by the NPSEM (Y, M, Z, C, A, W), with column names
#'   set appropriately based on the input data. Such a structure is a
...
fit_nuisance_d <- function(train_data,
  ...
  eif_data_train <- eif_data_train[C == 1, ]
