...
g_out <- fit_g_mech(train_data = data, valid_data = data, contrast = c(0, 1),
...
e_out <- fit_e_mech(train_data = data, valid_data = data, contrast = c(0, 1),
...
m_out <- fit_m_mech(train_data = data, valid_data = data, contrast = c(0, 1),
...
u_int_eif <- lapply(unique(data$Z), function(confounder_val) {
  ...
  data_z_interv[, `:=`(Z = confounder_val,
                       A = contrast[1],
                       U_pseudo = u_prime)]
